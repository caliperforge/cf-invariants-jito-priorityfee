...
// cf-invariants-jito-priorityfee port helper: anchor-lang 1.0.1's
// `solana_program` shim no longer re-exports `native_token::lamports_to_sol`.
// The single upstream use (a `msg!` line inside
// `transfer_priority_fee_tips`'s not-live-yet branch) is replaced with this
// local equivalent — `lamports / 1e9` — matching the standalone
// `solana-native-token` crate's `LAMPORTS_PER_SOL = 1_000_000_000`
// definition. Defined at module-root (outside `#[program] pub mod ...`) so
// Anchor's instruction-registration machinery does not see it.
#[inline(always)]
...
pub mod jito_priority_fee_distribution {
    ...
    //   anchor-lang 1.0.1's `solana_program` shim no longer re-exports the
    //   `hash` module. The standalone `solana-sha256-hasher` 3.x crate is the
    //   modular replacement and exposes the same `hashv(&[&[u8]]) -> Hash` API
    //   that `solana_program::hash::hashv` used to. Importing it directly here
    //   localizes the change to the `claim` instruction's merkle-leaf node hash.
    //   The other `solana_program::*` modules used by this program
    //   (`native_token`, `system_instruction`, `program`) ARE still re-exported
    //   by the anchor-lang 1.0.1 shim — the uses inside `transfer_priority_fee_tips`
    //   and the `lamports_to_sol` log line are preserved unchanged.
    use solana_sha256_hasher::hashv;

    use jito_programs_vote_state::VoteState;
    // cf-invariants-jito-priorityfee port: anchor-lang 1.0.1 keeps the
    // `solana_program` re-export inside its prelude (via
    // `anchor_lang::solana_program::*`), but the bare `solana_program::*`
    // path is no longer in scope by default. Bring it in explicitly so the
    // remaining upstream `solana_program::native_token` /
    // `system_instruction` / `program` references (inside
    // `transfer_priority_fee_tips` and the `lamports_to_sol` log line)
    // compile without touching the call sites themselves.
    use anchor_lang::solana_program;
    // Module-root helper (above `#[program]`) replaces the upstream
    // `use solana_program::native_token::lamports_to_sol;` import.
    use super::lamports_to_sol;
    ...
    pub fn claim(ctx: Context<Claim>, _bump: u8, amount: u64, proof: Vec<[u8; 32]>) -> Result<()> {
        ...
        //
        // cf-invariants-jito-priorityfee port: explicit `as &[u8]` slice coercions
        // resolve an E0308 the upstream `solana_program::hash::hashv` chain avoided
        // purely by inference. With all hashv args explicitly typed as `&[u8]`, the
        // slice element type unifies cleanly across the size-1 `[0u8]` prefix array
        // and the size-32 claimant/amount arrays.
        let node = &hashv(&[
            &[0u8] as &[u8],
            &hashv(&[
                &claimant_account.key().to_bytes() as &[u8],
                &amount.to_le_bytes() as &[u8],
            ])
            .to_bytes() as &[u8],
        ]);
    ...
    pub fn transfer_priority_fee_tips(
        ...
        // cf-invariants-jito-priorityfee PLANTED-BUG SITE
        // ----------------------------------------------
        // Class: transfer_increments_total_state_update
        // Invariant: invariant_transfer_priority_fee_tips_increments_total
        // Bug:     the upstream line below has been removed:
        //
        //     ctx.accounts
        //         .priority_fee_distribution_account
        //         .increment_total_lamports_transferred(lamports)?;
        //
        // The system-program lamport transfer (below) still runs normally,
        // so the SVM tx-runtime balance check is NOT tripped. What's lost
        // is the structural commit of the new `total_lamports_transferred`
        // field on the PriorityFeeDistributionAccount. After this transfer
        // succeeds, the fixture's `expected_total` advances by `lamports`
        // while the on-chain account stays at its prior value → invariant
        // violation on the first successful call.
        // (Single-site planted bug — no other changes vs. the clean ref.)
